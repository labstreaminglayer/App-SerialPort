...
void read_thread(HANDLE hPort, int comPort, int baudRate, int samplingRate, int chunkSize,
	...
	try {
		...
		// create streaminfo
		lsl::stream_info info(streamName, "Raw", 1, samplingRate, lsl::cf_int16,
			std::string("SerialPort_") + streamName);
		...
		while (!shutdown) {
			...
			sample = byte;
			// transmit it
			if (bytes_read) outlet.push_sample(&sample);
		}
	} catch (std::exception &e) {
		// any other error
		QMessageBox::critical(nullptr, "Error",
			QStringLiteral("Error during processing: ") + e.what(), QMessageBox::Ok);
	}
	CloseHandle(hPort);
}
